...
void publish_status(t_jit_handpose* x, bool ready, int code, std::string text) {
    if (!x || !x->state) return;
    {
...
Result copy_result(const MpHandLandmarkerResult& source, Frame&& frame) {
    Result destination;
    ...
    const uint32_t count = std::max(
        source.handedness_count,
        std::max(source.hand_landmarks_count, source.hand_world_landmarks_count));
    destination.hands.resize(count);
    for (uint32_t hand = 0; hand < count; ++hand) {
        auto& output = destination.hands[hand];
        if (hand < source.handedness_count) {
            const auto& categories = source.handedness[hand];
            if (categories.categories && categories.categories_count > 0) {
                const auto& category = categories.categories[0];
                if (category.category_name) output.handedness = category.category_name;
                output.handedness_score = category.score;
            }
        }
        if (hand < source.hand_landmarks_count) {
            const auto& landmarks = source.hand_landmarks[hand];
        ...
        if (hand < source.hand_world_landmarks_count) {
            const auto& landmarks = source.hand_world_landmarks[hand];
            output.world.reserve(landmarks.landmarks_count);
            for (uint32_t i = 0; i < landmarks.landmarks_count; ++i) {
                const auto& p = landmarks.landmarks[i];
                output.world.push_back({p.x, p.y, p.z});
            }
        }
    }
    return destination;
...
MpHandLandmarkerPtr create_landmarker(const RuntimeConfig& config,
    ...
    options.base_options.app_id = "jit.handpose";
    options.base_options.app_version = "1.1.0";
    options.running_mode = MP_RUNNING_MODE_VIDEO;
    options.num_hands = config.hands;
    options.min_hand_detection_confidence = config.detection_threshold;
    options.min_hand_presence_confidence = config.presence_threshold;
    options.min_tracking_confidence = config.tracking_threshold;
    MpHandLandmarkerPtr landmarker = nullptr;
    char* message = nullptr;
    const MpStatus status = MpHandLandmarkerCreate(&options, &landmarker, &message);
    if (status != kMpOk) {
        error_code = status;
...
void close_landmarker(MpHandLandmarkerPtr& landmarker) {
    if (!landmarker) return;
    char* message = nullptr;
    MpHandLandmarkerClose(landmarker, &message);
    if (message) MpErrorFree(message);
    landmarker = nullptr;
}

void worker_main(t_jit_handpose* x) {
    State* state = x->state;
    MpHandLandmarkerPtr landmarker = nullptr;
    for (;;) {
        Frame frame;
        ...
        if (restart) {
            ...
            if (config.model_path.empty()) {
                publish_status(x, false, kMpNotFound,
                               "hand_landmarker.task was not found on Max's search path");
                continue;
            }
            int code = 0;
            std::string error;
        ...
        MpHandLandmarkerResult media_result{};
        message = nullptr;
        ...
        MpHandLandmarkerCloseResult(&media_result);
        {
            std::lock_guard<std::mutex> lock(state->mutex);
...
bool render_matrix(void* matrix, const Result& result, bool show_background) {
    ...
    for (size_t h = 0; h < result.hands.size(); ++h) {
        const auto& points = result.hands[h].normalized;
...
bool ensure_annotated_matrix(t_jit_handpose* x, int width, int height) {
...
void output_status(t_jit_handpose* x, bool force) {
...
void output_qelem_method(t_jit_handpose* x) {
    if (!x || !x->state) return;
    ...
    t_atom hands_atoms[2];
    atom_setlong(hands_atoms, static_cast<long>(result.hands.size()));
    atom_setlong(hands_atoms + 1, static_cast<t_atom_long>(result.timestamp_ms));
    outlet_anything(x->metadata_outlet, gensym("hands"), 2, hands_atoms);
    ...
    for (size_t hand = result.hands.size(); hand-- > 0;) {
        const auto& item = result.hands[hand];
        t_atom handedness[3];
        atom_setlong(handedness, static_cast<long>(hand));
        atom_setsym(handedness + 1, gensym(item.handedness.c_str()));
        atom_setfloat(handedness + 2, item.handedness_score);
        outlet_anything(x->metadata_outlet, gensym("handedness"), 3, handedness);
        output_point_list(x->world_outlet, gensym("world"), hand, item.world);
        output_point_list(x->landmarks_outlet, gensym("landmarks"), hand, item.normalized);
    }
}
...
void matrix_method(t_jit_handpose* x, t_symbol*, long argc, t_atom* argv) {
...
void start_method(t_jit_handpose* x) {
    if (!x || !x->state) return;
...
void stop_method(t_jit_handpose* x) {
    if (!x || !x->state) return;
...
void reset_method(t_jit_handpose* x) { if (x && x->state) restart(x); }
void model_method(t_jit_handpose* x, t_symbol* model) {
...
void hands_method(t_jit_handpose* x, long hands) {
    if (!x || !x->state) return;
    if (hands < 1 || hands > kMaximumHands) { object_error(reinterpret_cast<t_object*>(x), "hands must be 1-4"); return; }
    { std::lock_guard<std::mutex> lock(x->state->mutex); x->state->config.hands = static_cast<int>(hands); }
    restart(x);
}
void thresholds_method(t_jit_handpose* x, t_symbol*, long argc, t_atom* argv) {
...
void status_method(t_jit_handpose* x) { if (x && x->state) output_status(x, true); }
void background_method(t_jit_handpose* x, long enabled) {
    if (x) x->show_background = enabled != 0;
}
void mirror_method(t_jit_handpose* x, long enabled) {
    if (x) x->mirror = enabled != 0;
}

void assist_method(t_jit_handpose*, void*, long message, long index, char* text) {
...
void free_method(t_jit_handpose* x) {
    if (!x) return;
    if (x->state) {
...
void* new_method(t_symbol*, long argc, t_atom* argv) {
    auto* x = static_cast<t_jit_handpose*>(object_alloc(g_class));
    if (!x) return nullptr;
    ...
    if (argc > 0 && (atom_gettype(argv) == A_LONG || atom_gettype(argv) == A_FLOAT))
        x->state->config.hands = std::clamp<int>(atom_getlong(argv), 1, 4);
    resolve_file("hand_landmarker.task", x->state->config.model_path);
...
void ext_main(void*) {
    t_class* klass = class_new("jit.handpose", reinterpret_cast<method>(new_method),
        reinterpret_cast<method>(free_method), sizeof(t_jit_handpose), nullptr, A_GIMME, 0);
    ...
    class_addmethod(klass, reinterpret_cast<method>(hands_method), "hands", A_LONG, 0);
