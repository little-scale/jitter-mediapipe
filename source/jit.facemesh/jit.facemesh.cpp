...
struct FaceResult {
    ...
    uint32_t transform_rows = 0;
    uint32_t transform_cols = 0;
...
typedef struct _jit_facemesh {
    t_object object;
    void* landmarks_outlet = nullptr;
    void* blendshapes_outlet = nullptr;
...
void publish_status(t_jit_facemesh* x, bool ready, int code, std::string text) {
    if (!x || !x->state) return;
    {
...
Result copy_result(const MpFaceLandmarkerResult& source, Frame&& frame) {
    Result destination;
    ...
    const uint32_t count = std::max(
        source.face_landmarks_count,
        std::max(source.face_blendshapes_count,
                 source.facial_transformation_matrixes_count));
    destination.faces.resize(count);
    for (uint32_t face = 0; face < count; ++face) {
        auto& output = destination.faces[face];
        if (face < source.face_landmarks_count) {
            const auto& landmarks = source.face_landmarks[face];
            ...
            for (uint32_t i = 0; i < landmarks.landmarks_count; ++i) {
                ...
                output.normalized.push_back({p.x, p.y, p.z});
            }
        }
        if (face < source.face_blendshapes_count) {
            const auto& categories = source.face_blendshapes[face];
            output.blendshapes.reserve(categories.categories_count);
            for (uint32_t i = 0; i < categories.categories_count; ++i) {
                const auto& category = categories.categories[i];
                output.blendshapes.push_back(
                    {category.index, category.score,
                     category.category_name ? category.category_name : ""});
            }
        }
        if (face < source.facial_transformation_matrixes_count) {
            const auto& matrix = source.facial_transformation_matrixes[face];
            output.transform_rows = matrix.rows;
            output.transform_cols = matrix.cols;
            if (matrix.data)
                output.transform.assign(matrix.data,
                    matrix.data + static_cast<size_t>(matrix.rows) * matrix.cols);
        }
    }
    return destination;
}

MpFaceLandmarkerPtr create_landmarker(const RuntimeConfig& config,
    ...
    options.base_options.app_id = "jit.facemesh";
    options.base_options.app_version = "1.1.0";
    options.running_mode = MP_RUNNING_MODE_VIDEO;
    options.num_faces = config.faces;
    options.min_face_detection_confidence = config.detection_threshold;
    options.min_face_presence_confidence = config.presence_threshold;
    ...
    const MpStatus status = MpFaceLandmarkerCreate(&options, &landmarker, &message);
    if (status != kMpOk) {
        error_code = status;
...
void close_landmarker(MpFaceLandmarkerPtr& landmarker) {
    if (!landmarker) return;
    char* message = nullptr;
    MpFaceLandmarkerClose(landmarker, &message);
    if (message) MpErrorFree(message);
    landmarker = nullptr;
}

void worker_main(t_jit_facemesh* x) {
    State* state = x->state;
    MpFaceLandmarkerPtr landmarker = nullptr;
    for (;;) {
        Frame frame;
        ...
        if (restart) {
            ...
            if (config.model_path.empty()) {
                publish_status(x, false, kMpNotFound,
                               "face_landmarker.task was not found on Max's search path");
                continue;
            }
            int code = 0;
            std::string error;
        ...
        MpFaceLandmarkerResult media_result{};
        message = nullptr;
        ...
        MpFaceLandmarkerCloseResult(&media_result);
        {
            std::lock_guard<std::mutex> lock(state->mutex);
...
constexpr int kConnections[][2] = {
    ...
    {386,385},{385,384},{384,398},{398,362},{276,283},{283,282},
    {282,295},{295,285},{300,293},{293,334},{334,296},{296,336},
    {33,7},{7,163},{163,144},{144,145},{145,153},{153,154},{154,155},
    {155,133},{33,246},{246,161},{161,160},{160,159},{159,158},
    {158,157},{157,173},{173,133},{46,53},{53,52},{52,65},{65,55},
    {70,63},{63,105},{105,66},{66,107},{10,338},{338,297},{297,332},
    {332,284},{284,251},{251,389},{389,356},{356,454},{454,323},
    {323,361},{361,288},{288,397},{397,365},{365,379},{379,378},
    {378,400},{400,377},{377,152},{152,148},{148,176},{176,149},
    {149,150},{150,136},{136,172},{172,58},{58,132},{132,93},{93,234},
    {234,127},{127,162},{162,21},{21,54},{54,103},{103,67},{67,109},
    {109,10},{474,475},{475,476},{476,477},{477,474},{469,470},
    {470,471},{471,472},{472,469},
};
...
bool render_matrix(void* matrix, const Result& result, bool show_background) {
    ...
    const int line_radius = std::max(1, edge / 720);
    const int joint_radius = std::max(1, edge / 480);
    for (size_t h = 0; h < result.faces.size(); ++h) {
        const auto& points = result.faces[h].normalized;
        if (points.size() < 468) continue;
        const Color color = kColors[h % 4];
        for (const auto& connection : kConnections) {
            if (connection[0] >= static_cast<int>(points.size()) ||
                connection[1] >= static_cast<int>(points.size())) continue;
            int x0, y0, x1, y1;
...
bool ensure_annotated_matrix(t_jit_facemesh* x, int width, int height) {
...
void output_point_list(void* outlet, t_symbol* selector, long hand_index,
                       const std::vector<Point3>& points) {
    std::vector<t_atom> atoms(1 + points.size() * 3);
    atom_setlong(atoms.data(), hand_index);
    for (size_t i = 0; i < points.size(); ++i) {
        atom_setfloat(atoms.data() + 1 + i * 3, points[i].x);
        atom_setfloat(atoms.data() + 2 + i * 3, points[i].y);
        atom_setfloat(atoms.data() + 3 + i * 3, points[i].z);
    }
    outlet_anything(outlet, selector, static_cast<short>(atoms.size()), atoms.data());
}

void output_blendshapes(void* outlet, long face_index,
                        const std::vector<Blendshape>& blendshapes) {
    std::vector<t_atom> atoms(1 + blendshapes.size() * 2);
    atom_setlong(atoms.data(), face_index);
    for (size_t i = 0; i < blendshapes.size(); ++i) {
        atom_setsym(atoms.data() + 1 + i * 2,
                    gensym(blendshapes[i].name.c_str()));
        atom_setfloat(atoms.data() + 2 + i * 2, blendshapes[i].score);
    }
    outlet_anything(outlet, gensym("blendshapes"),
                    static_cast<short>(atoms.size()), atoms.data());
}

void output_transform(void* outlet, long face_index, const FaceResult& face) {
    if (face.transform.empty()) return;
    std::vector<t_atom> atoms(3 + face.transform.size());
    atom_setlong(atoms.data(), face_index);
    atom_setlong(atoms.data() + 1, face.transform_rows);
    atom_setlong(atoms.data() + 2, face.transform_cols);
    for (size_t i = 0; i < face.transform.size(); ++i)
        atom_setfloat(atoms.data() + 3 + i, face.transform[i]);
    outlet_anything(outlet, gensym("transform"),
                    static_cast<short>(atoms.size()), atoms.data());
}

void output_status(t_jit_facemesh* x, bool force) {
...
void output_qelem_method(t_jit_facemesh* x) {
    if (!x || !x->state) return;
    ...
    t_atom faces_atoms[2];
    atom_setlong(faces_atoms, static_cast<long>(result.faces.size()));
    atom_setlong(faces_atoms + 1, static_cast<t_atom_long>(result.timestamp_ms));
    outlet_anything(x->metadata_outlet, gensym("faces"), 2, faces_atoms);
    if (result.faces.empty()) {
        t_atom timestamp;
        atom_setlong(&timestamp, static_cast<t_atom_long>(result.timestamp_ms));
        outlet_anything(x->blendshapes_outlet, gensym("clear"), 1, &timestamp);
        outlet_anything(x->landmarks_outlet, gensym("clear"), 1, &timestamp);
        return;
    }
    for (size_t face = result.faces.size(); face-- > 0;) {
        const auto& item = result.faces[face];
        output_transform(x->metadata_outlet, face, item);
        output_blendshapes(x->blendshapes_outlet, face, item.blendshapes);
        output_point_list(x->landmarks_outlet, gensym("landmarks"), face,
                          item.normalized);
    }
}
...
void matrix_method(t_jit_facemesh* x, t_symbol*, long argc, t_atom* argv) {
...
void start_method(t_jit_facemesh* x) {
    if (!x || !x->state) return;
...
void stop_method(t_jit_facemesh* x) {
    if (!x || !x->state) return;
...
void reset_method(t_jit_facemesh* x) { if (x && x->state) restart(x); }
void model_method(t_jit_facemesh* x, t_symbol* model) {
...
void faces_method(t_jit_facemesh* x, long faces) {
    if (!x || !x->state) return;
    if (faces < 1 || faces > kMaximumFaces) { object_error(reinterpret_cast<t_object*>(x), "faces must be 1-4"); return; }
    { std::lock_guard<std::mutex> lock(x->state->mutex); x->state->config.faces = static_cast<int>(faces); }
    restart(x);
}
void thresholds_method(t_jit_facemesh* x, t_symbol*, long argc, t_atom* argv) {
...
void status_method(t_jit_facemesh* x) { if (x && x->state) output_status(x, true); }
void background_method(t_jit_facemesh* x, long enabled) {
    if (x) x->show_background = enabled != 0;
}
void mirror_method(t_jit_facemesh* x, long enabled) {
    if (x) x->mirror = enabled != 0;
}

void assist_method(t_jit_facemesh*, void*, long message, long index, char* text) {
    if (message == ASSIST_INLET) { std::snprintf(text, 512, "Jitter matrix and control messages"); return; }
    static const char* labels[] = {"normalized face mesh", "blendshapes", "metadata and transforms", "annotated ARGB matrix"};
    std::snprintf(text, 512, "%s", labels[std::clamp<long>(index, 0, 3)]);
}

void free_method(t_jit_facemesh* x) {
    if (!x) return;
    if (x->state) {
...
void* new_method(t_symbol*, long argc, t_atom* argv) {
    auto* x = static_cast<t_jit_facemesh*>(object_alloc(g_class));
    if (!x) return nullptr;
    x->landmarks_outlet = x->blendshapes_outlet = x->metadata_outlet = nullptr;
    ...
    if (argc > 0 && (atom_gettype(argv) == A_LONG || atom_gettype(argv) == A_FLOAT))
        x->state->config.faces = std::clamp<int>(atom_getlong(argv), 1, 4);
    resolve_file("face_landmarker.task", x->state->config.model_path);
    x->annotated_outlet = outlet_new(x, "jit_matrix");
    x->metadata_outlet = outlet_new(x, nullptr);
    x->blendshapes_outlet = outlet_new(x, nullptr);
...
void ext_main(void*) {
    t_class* klass = class_new("jit.facemesh", reinterpret_cast<method>(new_method),
        reinterpret_cast<method>(free_method), sizeof(t_jit_facemesh), nullptr, A_GIMME, 0);
    ...
    class_addmethod(klass, reinterpret_cast<method>(faces_method), "faces", A_LONG, 0);
